...
var WidgetContainer = function (_a) {
    ...
    var _j = useState({ x: 0, y: 0, mouseX: 0, mouseY: 0 }), dragStart = _j[0], setDragStart = _j[1];
    var _k = useState({ width: 0, height: 0, mouseX: 0, mouseY: 0 }), resizeStart = _k[0], setResizeStart = _k[1];
    var tempPositionRef = useRef(layout.position);
    var tempSizeRef = useRef(layout.size);
    ...
    return (jsxs("div", __assign({ ref: containerRef, className: "widget-container ".concat(isEditMode ? 'edit-mode' : '', " ").concat(isDragging ? 'dragging' : '', " ").concat(className), style: containerStyle, onMouseDown: handleMouseDown }, { children: [jsx("div", __assign({ className: "widget-content" }, { children: layout.children })), isEditMode && (jsx("div", { className: "widget-resize-handle", onMouseDown: handleResizeStart }))] })));
};
...
var WidgetGrid = function (_a) {
    ...
    return (jsx("div", __assign({ className: "widget-grid ".concat(isEditMode ? 'edit-mode' : ''), style: gridStyle }, { children: layouts.map(function (layout) { return (jsx(WidgetContainer, { layout: layout, isEditMode: isEditMode, onMove: handleMove, onResize: handleResize, gridSize: gridSize, maxWidth: width, maxHeight: height }, layout.id)); }) })));
};
...
var WidgetDashboard = function (_a) {
    ...
    var _f = useState(layouts), layoutsState = _f[0], setLayoutsState = _f[1];
